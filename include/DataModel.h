...
{
    virtual void  Write(void* ptr, Value const& value) const override { *(static_cast<T*>(ptr)) = static_cast<T>(value); }
...
template <typename TFieldTraits> struct ObjectDataTypeHandler<DataType::Value, TFieldTraits> : public IDataTypeHandler<DataType::Value>
{
    ...
    {
        // typename TFieldTraits::TPropertySetter;
        using FieldValueType = typename FieldTypeTraits::ValueType;
        (static_cast<TOwner*>(ptr)->*(TFieldTraits::TPropertySetter()))(value.convert<FieldValueType>());
    }
...
template <typename T> struct ReflectionBase::TypeTraits<T&, std::enable_if_t<Value::Supported<T>::value>>
{
    ...
    static constexpr std::string_view Name() { return typeid(T).name(); }
