...
template <typename T> static void RunTestCase(TestCase const& tc, std::vector<uint8_t>& data, std::string const& /*name*/)
{
    ...
    {
        fmt::print(stderr, "Testcase[{}]:{}, Exception: {}\n", typeid(T).name(), tc.desc, ex.what());
        // lines.push_back(fmt::format("Testcase[{}]:{}, Exception: {}", name, tc.desc, ex.what()));
    }
}

template <typename T> static void RunTestCases(std::vector<TestCase> const& cases, std::string const& name)
{
    {
        ...
        for (auto const& tc : cases) { RunTestCase<T>(tc, data, name); }
...
TEST_CASE("Binary", "[Protocol]")
{
    ...
    SECTION("WithPrimitives64Bit")
    {
        RunTestCases<WithPrimitives64Bit>(
            {
                {.json = R"({"f1": -1})", .desc = "int64-1", .valid = true},
        ...
        RunTestCases<MultiAttributed>(
            {
                ...
                {
                    .json  = R"({"timestamp": "2020-01-02:03:04:05.600", "uuid": "01234567-8901-2345-6789-012345678901", "f1": {}})",
                    .desc  = "",
                    .valid = true,
                },
            },
    ...
    SECTION("WithVariant")
    {
        RunTestCases<WithVariant>(
            {
                {
                    .json  = R"({"f1": {"0": {"f1": {}}}, "f2": {"0": 1}, "f3": {"0": {"1": 0.1}}, "f4": {"0": 2}})",
                    .desc  = "0",
                    .valid = true,
                },
                {
                    .json  = R"({"f1": {"1": {"f1": {}}}, "f2": {"1": "a"}, "f3": {"1": "d"}, "f4": {"1": {"f1": {}}}})",
                    .desc  = "1",
                    .valid = true,
                },
                {
                    .json  = R"({"f1": {"2": {"f1": {}}}, "f2": {"2": "bcd"}, "f3": {"2": "efg"}, "f4": {"0": 3}})",
                    .desc  = "2",
                    .valid = true,
                },
                {
